"""
Test para verificar el análisis de complejidad de una función con ciclo FOR
que verifica condiciones en un array y cuenta elementos.

Pseudocódigo evaluado:
verificarCondiciones(arr, n) begin encontrado 🡨 F contador 🡨 0 for i 🡨 1 to n do begin if (arr[i] > 10) then begin encontrado 🡨 T contador 🡨 contador + 1 end end if (encontrado = T) then begin return contador end else begin return 0 end end
"""

from services.analysis_service import analyze_pseudocode


def test_for_loop_verificar_condiciones():
    """
    PRUEBA: Función verificarCondiciones con ciclo FOR
    
    Verifica que una función con un ciclo FOR que verifica condiciones en un array
    genere la complejidad O(n), Ω(n) y Θ(n).
    """
    # Pseudocódigo a evaluar
    pseudocode = "verificarCondiciones(arr, n) begin encontrado 🡨 F contador 🡨 0 for i 🡨 1 to n do begin if (arr[i] > 10) then begin encontrado 🡨 T contador 🡨 contador + 1 end end if (encontrado = T) then begin return contador end else begin return 0 end end"
