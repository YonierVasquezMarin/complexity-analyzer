"""
Test para verificar el análisis de complejidad de un ciclo FOR que procesa
un subarreglo y suma sus elementos.

Pseudocódigo evaluado:
procesarSubarreglo(A, n) begin temp 🡨 A[1..n] for i 🡨 1 to n do begin x 🡨 temp[i] suma 🡨 suma + x end return suma end
"""

from services.analysis_service import analyze_pseudocode


def test_for_loop_subarray_processing():
    """
    PRUEBA: Ciclo FOR que procesa un subarreglo y suma sus elementos
    
    Verifica que un ciclo FOR que procesa un subarreglo de tamaño n
    genere la complejidad O(n), Ω(n) y Θ(n).
    """
    # Pseudocódigo a evaluar
    pseudocode = "procesarSubarreglo(A, n) begin temp 🡨 A[1..n] for i 🡨 1 to n do begin x 🡨 temp[i] suma 🡨 suma + x end return suma end"
