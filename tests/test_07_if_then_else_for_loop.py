"""
Test para verificar el análisis de complejidad de una estructura condicional IF-THEN-ELSE
con un ciclo FOR en el bloque THEN y una asignación en el bloque ELSE.

Pseudocódigo evaluado:
if (x > 0) then begin for i 🡨 1 to n do begin y 🡨 y + 1 end end else begin z 🡨 z + 1 end
"""

from services.analysis_service import analyze_pseudocode


def test_if_then_else_for_loop():
    """
    PRUEBA: Estructura condicional IF-THEN-ELSE con ciclo FOR en THEN
    
    Verifica que una estructura condicional IF-THEN-ELSE que contiene un ciclo FOR
    en el bloque THEN genere la complejidad O(n), Ω(n) y Θ(n).
    """
    # Pseudocódigo a evaluar
    pseudocode = "if (x > 0) then begin for i 🡨 1 to n do begin y 🡨 y + 1 end end else begin z 🡨 z + 1 end"
