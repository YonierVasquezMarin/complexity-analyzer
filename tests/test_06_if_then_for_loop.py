"""
Test para verificar el análisis de complejidad de una estructura condicional IF-THEN
con un ciclo FOR dentro del bloque.

Pseudocódigo evaluado:
if (x > 0) then begin for i 🡨 1 to n do begin y 🡨 y + 1 end end
"""

from services.analysis_service import analyze_pseudocode


def test_if_then_for_loop():
    """
    PRUEBA: Estructura condicional IF-THEN con ciclo FOR
    
    Verifica que una estructura condicional IF-THEN que contiene un ciclo FOR
    genere la complejidad O(n), Ω(n) y Θ(n).
    """
    # Pseudocódigo a evaluar
    pseudocode = "if (x > 0) then begin for i 🡨 1 to n do begin y 🡨 y + 1 end end"
